...
obsid_file = read_csv(f'{base}/{folder}/ObsFlux_test.csv')
...
f = open(f'{projekt_1}/NuSTAR/{folder}/tbvarabs_test.csv','+w')  # creates a csv document with all outputs
f.write('"Name"\t"obsid"\t"dist[Mpc]"\t"type"\t"epoch"\t"exp_timeA(ks)"\t"exp_timeB(ks)"\t"mean_nH(1e22atoms_cm-2)"\t"RA"\t"DEC"\t"dateXray"\t"countsA"\t"countsB"\t"line_of_sight"\t"PhoIndex"\t"confInt(1.6σ)"\t"norm"\t"confInt(1.6σ)"\t"cstat"\t"chi2"\t"dof"\t"PhoInd_2_10"\t"confInt(1.6σ)_2_10"\t"PL_norm_2_10"\t"absflux_2_10"\t"flux_confInt_2_10(1.6σ)"\t"Lum_2_10"\t"lum_confInt_2_10(1.6σ)"\t"flux_confInt_2_10(3σ)"\t"lum_confInt_2_10(3σ)"\t"PhoInd_3_78"\t"confInt(1.6σ)_3_78"\t"PL_norm_3_78"\t"absflux_3_78"\t"flux_confInt_3_78(1.6σ)"\t"Lum_3_78"\t"lum_confInt_3_78(1.6σ)"\t"flux_confInt_3_78(3σ)"\t"lum_confInt_3_78(3σ)"\t"PhoInd_2_78"\t"confInt(1.6σ)_2_78"\t"PL_norm_2_78"\t"absflux_2_78"\t"flux_confInt_2_78(1.6σ)"\t"Lum_2_78"\t"lum_confInt_2_78(1.6σ)"\t"flux_confInt_2_78(3σ)"\t"lum_confInt_2_78(3σ)"\t"PhoInd_10_30"\t"confInt(1.6σ)_10_30"\t"PL_norm_10_30"\t"absflux_10_30"\t"flux_confInt_10_30(1.6σ)"\t"Lum_10_30"\t"lum_confInt_10_30(1.6σ)"\t"flux_confInt_10_30(3σ)"\t"lum_confInt_10_30(3σ)"\t"PhoInd_10_78"\t"confInt(1.6σ)_10_78"\t"PL_norm_10_78"\t"absflux_10_78"\t"flux_confInt_10_78(1.6σ)"\t"Lum_10_78"\t"lum_confInt_10_78(1.6σ)"\t"flux_confInt_10_78(3σ)"\t"lum_confInt_10_78(3σ)"\n') #90
                    ...
                    for j in range(3):
                        ...
                        if ma.is_masked(obsid_file['obsflux_2_10'].data[i]) or obsid_file['obsflux_2_10'].data[i]==' ':
                            ...
                            if float(obsid_file['flux_confInt_2_10(3σ)'][i].split(',')[0].replace('(',''))==0.0 or float(obsid_file['flux_confInt_2_10(3σ)'][i].split(',')[0].replace('(',''))==1.0:
                                ...
                                mo(51).values = 1
                                mo(52).values = 1e-4"""
                                detected='no'## TODO: setting this as default
...
os.chdir(projekt_1+'/pipelines/pyxspec')

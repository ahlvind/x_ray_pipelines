...
# –––––––––––––––––––
if telescope=='SWIFT':
    obsid_file = ascii.read(f'{base}/{telescope}/{folder}/spec_fitting.txt')
    obsid = obsid_file['obsid']
else:
    obsid_file = ascii.read(f'{base}/{telescope}/main_list_EjGalax.txt') #main_list_EjGALAX
    with open(f'{base}/{telescope}/main_list_EjGalax.txt') as p:
        lines = p.readlines()
    obsid = []
    for line in lines:
    ...
    if (spec_file_name in list_files_in_repro):
        ...
        if counts<1:
            ...
            try:
                ...
                print("Fit.perform failed without cflux. Error: {e}")
            ...
            if try1 == True:
                ...
                # –––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
                for m in range(2):
                    print(f'ENERGY INTERVAL: {E_int[m][0]} {E_int[m][1]} ')
                    ...
                    if counts2<-100:
                        ...
                        try:
                            ...
                            print(f"Error in Fit.steppar for 2.706, doing steppar instead. Error: {e}")
                            try:
                                cflux, cflux_under, cflux_over = call_steppar(cstat=2.706,paramNr=3,step_start=-16, step_end=-10)
                            except Exception as e:
                                print("Error in steppar, printing cflux=0. Error: {e}")
                        ...
                        try:
                            ...
                            # if flux is an even number, do steppar instead
                            if (cflux % 1==0) or cflux<-20:
                                raise Exception("The result is not sufficient for 3σ")

                            
                        
                        # if fit not possible to steppar directly
                        except Exception as e:
                            print(f"Error in Fit.steppar for 9.0: {e}")
                            ...
                            try:
                                ...
                                print("Error in steppar for 3σ, printing cflux=0. Error: {e}")

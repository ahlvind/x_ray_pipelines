                        ...
                        if counts2<1:
                            ...
                            try:
                                ...
                                if (cflux % 1==0) or cflux<-20:
                                    try: # try doing steppar if fitting didn't work
                                        cflux, cflux_under, cflux_over = call_steppar(cstat=2.706,paramNr=46,step_start=-18, step_end=-8)
                                    except:
                            ...
                            if try2 == False: # not worth trying fitting for 3σ if 1σ didn't work
                                ...
                                try:
                                    ...
                                    # if flux is an even number, do steppar instead
                                    if (cflux % 1==0) or cflux<-20:
                                        cflux, cflux_under, cflux_over = call_steppar(cstat=9.0,paramNr=46,step_start=-18, step_end=-8)
                                        
                                        if cflux_under==0 or cflux_under == 1:
                                            cflux, cflux_under, cflux_over = call_steppar(cstat=7.74,paramNr=46,step_start=-18, step_end=-8)
                                            cflux_under = 0
